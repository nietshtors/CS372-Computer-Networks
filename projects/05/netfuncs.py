...
def value_to_ipv4(addr):
    ...
    return '.'.join([f'{int((addr >> 8*i) & 255)}' for i in range(3, -1, -1)])
...
def find_router_for_ip(routers, ip):
    ...
    for router, d in routers.items():
        if ips_same_subnet(ip, router, d['netmask']):
            return router
    return None
...
def my_tests():
    ...
    print('value_to_ipv4()')
    print(value_to_ipv4(4294901760) == "255.255.0.0")
    print(value_to_ipv4(16909060) == "1.2.3.4")

    print('get_subnet_mask_value()')
    print(get_subnet_mask_value('/16') == 0xffff0000)
